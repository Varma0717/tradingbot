...
def dashboard():
    ...
    active_strategies = Strategy.query.filter_by(user_id=current_user.id, is_active=True).all()
    ...
    active_positions = [
        {'symbol': 'RELIANCE', 'qty': 10, 'avg_price': 2450.00, 'ltp': 2465.00, 'pnl': 150.00},
        {'symbol': 'TCS', 'qty': 20, 'avg_price': 3300.00, 'ltp': 3280.00, 'pnl': -400.00},
    ]
...
def strategies():
    ...
    return render_template('user/strategies.html', 
                         title='Strategies', 
...
def execute_trade():
    try:
        ...
        if not is_paper and not current_user.has_pro_plan:
            if request.is_json:
                return jsonify({'success': False, 'message': 'Pro plan required for real trading'}), 403
...
def toggle_strategy(strategy_id):
    try:
        ...
        return jsonify({'success': True, 'message': f'Strategy {"activated" if strategy.is_active else "deactivated"}'})
    except Exception as e:
        return jsonify({'success': False, 'message': str(e)}), 500
...
def create_strategy():
    try:
        ...
        if request.is_json:
            return jsonify({'success': True, 'message': 'Strategy created successfully', 'strategy_id': strategy.id})
...
def api_dashboard_data():
    ...
    try:
        # Get fresh data
        last_trades = Trade.query.filter_by(user_id=current_user.id)\
